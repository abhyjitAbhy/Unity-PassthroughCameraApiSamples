...
namespace SmartMove.AI
{
    public sealed class CustomImageSegmentationVisualizer : MonoBehaviour
    {
#if MRUK_INSTALLED
        ...
        /// <summary>
        /// Hard reset: destroys all pooled and live box instances and clears the
        /// component cache. Call this after an unlock cycle so stale locked-state
        /// objects are not reused by the next detection pass.
        /// </summary>
        public void ResetState()
        {
            // Destroy live boxes (may still be parented to world after lock)
            foreach (var g in _live)
            {
                if (g)
                {
                    _componentCache.Remove(g);
                    Destroy(g);
                }
            }
            _live.Clear();

            // Destroy pooled boxes — they may carry IsLocked=true marker state
            while (_pool.Count > 0)
            {
                var g = _pool.Dequeue();
                if (g)
            ...
            // Clear caches so next Draw3D starts clean
